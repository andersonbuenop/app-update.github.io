...
function Normalize-Version {
    ...
    if ($AppName) {
        ...
        if ($normApp -eq 'git') {
            # Remover sufixos como .windows.1, .windows.2
            return $Version -replace '\.windows\.\d+$', ''
        }
    }
...
function Resolve-AppInfoOnline {
    ...
    if ($AppSources.ContainsKey($normalized)) {
        ...
        # 4) Type Redirect
        if ($src.Type -eq 'Redirect') {
            Write-Host "   [Redirect] Redirecionando para: $($src.RedirectTo)"
            return (Resolve-AppInfoOnline -RawName $src.RedirectTo)
        }
...
function Get-NormalizedAppName {
    ...
    # Special handling for Java versions - preserve version numbers
    if ($name -match 'java.*se.*development.*kit.*8u') {
        return 'java se development kit 8'
    }
    if ($name -match 'java.*se.*development.*kit.*17') {
        return 'java se development kit 17'
    }
    if ($name -match 'java.*se.*development.*kit.*21') {
        return 'java se development kit 21'
    }
    
    # Special handling for .NET versions - preserve version numbers
    if ($name -match '\.net.*4\.6\.1') {
        return '.net 4.6.1'
    }
    if ($name -match '\.net.*4\.8\.1') {
        return '.net 4.8.1'
    }
    if ($name -match '\.net.*8\.0') {
        return '.net 8.0'
    }
    if ($name -match '\.net.*8') {
        return '.net 8'
    }
    if ($name -match 'java.*se.*8u') {
        return 'java se 8u'
    }
    if ($name -match 'java.*se.*17') {
        return 'java se 17'
    }
    if ($name -match 'java.*se.*21') {
        return 'java se 21'
    }
    if ($name -match 'jdk.*8u') {
        return 'jdk 8u'
    }
    if ($name -match 'jdk.*17') {
        return 'jdk 17'
    }
    if ($name -match 'jdk.*21') {
        return 'jdk 21'
    }
    if ($name -match 'java.*8u') {
        return 'java 8u'
    }
    if ($name -match 'java.*17') {
        return 'java 17'
    }
    if ($name -match 'java.*21') {
        return 'java 21'
    }
...
# Detecta se é processamento individual (CSV com apenas 1 app único)
...
if ($uniqueApps.Count -eq 1) {
    ...
    Write-Host "[Info] Detectado processamento individual (1 app único encontrado)" -ForegroundColor Yellow
}

# Define arquivo de saída baseado no tipo de processamento
if ($isIndividualProcessing) {
...
if (Test-Path $outPath) {
    ...
    Write-Host "[Info] Carregando dados existentes de $filename..." -ForegroundColor Cyan
    try {
...
$brandFavicons = @{
    'google chrome'                 = 'https://www.google.com/chrome/static/images/favicons/favicon-32x32.png'
    ...
    'eclipse ide'                   = 'https://download.eclipse.org/eclipse.org-common/themes/solstice/public/images/logo/eclipse-400x400.png'
    ...
    'gitkraken'                     = 'https://www.gitkraken.com/favicon.ico'
    ...
    'tesseract ocr'                 = 'https://github.com/tesseract-ocr.png'
    'eclipse temurin jdk with hotspot' = 'https://adoptium.net/favicon.ico'
    ...
    'keepassxc'                     = 'https://keepassxc.org/favicon.ico'
    'dax studio'                    = 'https://daxstudio.org/img/favicon.ico'
    'tabular editor'                = 'https://www.tabulareditor.com/favicon.ico'
    'caseware idea'                 = 'https://www.google.com/s2/favicons?domain=caseware.com&sz=128'
}
