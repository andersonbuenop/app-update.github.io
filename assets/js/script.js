// LOG INICIAL PARA VERIFICAR SE O SCRIPT ESTÁ RODANDO
console.log('=== SCRIPT CARREGADO ===');

function initFromText(text) {
  const { data } = parseCsv(text);
  
  // Preserva observações existentes ao recarregar dados
  if (state.data && state.data.length > 0) {
    console.log('Preservando observações...');
    console.log('Dados existentes:', state.data.length);
    console.log('Novos dados:', data.length);
    
    data.forEach((newItem, index) => {
      const existingItem = state.data.find(item => item.AppName === newItem.AppName);
      if (existingItem && existingItem.Observation) {
        console.log(`Preservando observação de ${newItem.AppName}:`, existingItem.Observation);
        newItem.Observation = existingItem.Observation;
      }
    });
  }
...
function exportCurrentViewToXlsx() {
  ...
  const headers = ['AppName', 'appversion', 'LatestVersion', 'Website', 'InstalledVersion', 'Status', 'License', 'TipoApp', 'Observation', 'SourceKey', 'SearchUrl', 'IsNewVersion', 'SourceId', 'IconUrl', 'IsDeleted'];
...
function renderTable() {
  ...
  state.filtered.forEach((row, index) => {
    ...
    tr.innerHTML = `
      ...
      <td class="col-obs">${row.Observation || ''}</td>
...
function openEditModal(index) {
  ...
  console.log('Item encontrado no filtered:', item);
  
  if (!item) {
    console.log('ERRO: Item não encontrado no filtered para índice', index);
    return;
  }
  ...
  document.getElementById('editObservation').value = item.Observation || '';
...
function updateSingleApp(index) {
  const item = state.filtered[index];
  if (!item) return;
  
  const button = document.getElementById(`updateBtn-${index}`);
  const originalText = button.innerText;
  button.disabled = true;
  button.innerText = 'Check...';
  button.classList.add('btn-loading');
  
  fetch('/update-single-app', {
    method: 'POST',
    headers: {
  ...
  .then(data => {
    ...
    if (data.success) {
      console.log('Dados recebidos:', data.app);
      
      // Atualiza os dados do item na tabela principal (state.data)
      const originalItem = state.data.find(d => d.AppName === item.AppName);
      console.log('Item original encontrado:', originalItem);
      
      if (originalItem && data.app) {
        console.log('Atualizando item original...');
        originalItem.LatestVersion = data.app.latestVersion || data.app.LatestVersion;
        originalItem.Status = data.app.status || data.app.Status;
        originalItem.Website = data.app.website || data.app.Website;
        originalItem.InstalledVersion = data.app.installedVersion || data.app.InstalledVersion;
        originalItem.IsNewVersion = data.app.isNewVersion || 'false';
        
        console.log('Item original após atualização:', originalItem);
        
        // Atualiza também no filtered
        item.LatestVersion = originalItem.LatestVersion;
        item.Status = originalItem.Status;
        item.Website = originalItem.Website;
        item.InstalledVersion = originalItem.InstalledVersion;
        item.IsNewVersion = originalItem.IsNewVersion;
        
        console.log('Item filtered após atualização:', item);
      }
      
      // Restaura o estado do botão ANTES de re-renderizar
      const buttonToRestore = document.getElementById(`updateBtn-${index}`);
      if (buttonToRestore) {
        buttonToRestore.disabled = false;
        buttonToRestore.innerText = originalText;
        buttonToRestore.classList.remove('btn-loading');
      }
      
      // Recria a tabela completamente (abordagem simples)
      renderTable();
      updateChart();
      
      // Salva as alterações no arquivo principal para persistir após refresh
      saveDataToServer();
      
      // Adiciona observação com data/hora em vez de popup
      const now = new Date();
      const timestamp = now.toLocaleString('pt-BR');
      const observationText = `Check individual realizado em ${timestamp}`;
      
      // Mantém texto existente e adiciona nova observação
      if (item.Observation && item.Observation.trim()) {
        item.Observation = `${item.Observation} | ${observationText}`;
      } else {
        item.Observation = observationText;
      }
      
      // Salva novamente para incluir a observação
      saveDataToServer();
      
      // Re-renderiza a tabela para mostrar a observação atualizada
      renderTable();
      
      console.log(`App "${item.AppName}" checkado com sucesso! Observação adicionada.`);
    } else {
      alert(`Erro ao checkar app: ${data.error || 'Erro desconhecido'}`);
    }
  })
  .catch(error => {
    console.error('Erro na requisição:', error);
    alert('Erro de conexão ao checkar app.');
  });
}
...
// LOG PARA VERIFICAR SE O EVENTO DO FORMULÁRIO ESTÁ SENDO ADICIONADO
console.log('=== ADICIONANDO EVENTO DO FORMULÁRIO ===');

// Manipulador do formulário de edição
document.getElementById('editForm').addEventListener('submit', function (e) {
  e.preventDefault();
  
  console.log('=== INÍCIO SALVAMENTO DO MODAL ===');
  ...
  console.log('Índice original:', originalIndex);
  console.log('Item encontrado:', row);
  
  if (row) {
    console.log('Atualizando dados do row...');
    ...
    row.Observation = document.getElementById('editObservation').value;
    row.TipoApp = document.getElementById('editTipoApp').value || 'app comercial';
    
    console.log('Row após atualização:', row);
    ...
    // Se for marcado como app interno, exclui automaticamente da lista principal
    if ((row.TipoApp || '').toLowerCase() === 'app interno') {
      row.IsDeleted = 'True';
    }

    // Atualiza o item no array principal (state.data)
    const itemIndex = state.data.findIndex(item => item.AppName === row.AppName);
    if (itemIndex !== -1) {
      console.log('Atualizando item no índice', itemIndex, 'com:', row);
      state.data[itemIndex] = row;
    }

    // Aplica filtros e recria tabela
    applyFilters();
    renderTable();
    updateChart();
    
    // Salva no servidor e fecha o modal
    saveDataToServer()
      .then(() => {
        closeModal();
      })
    ...
    console.log('=== FIM SALVAMENTO DO MODAL ===');
  } else {
    console.log('ERRO: Item não encontrado para o índice', originalIndex);
    saveDataToServer()
      .then(() => {
        closeModal();
      })
...
function saveDataToServer() {
  const headers = ['AppName', 'appversion', 'LatestVersion', 'Website', 'InstalledVersion', 'Status', 'License', 'SourceKey', 'SearchUrl', 'Observation', 'IsNewVersion', 'SourceId', 'IconUrl', 'TipoApp', 'IsDeleted'];
...
if (deleteBtn) {
  deleteBtn.addEventListener('click', () => {
    ...
    if (mode === 'restore' && (row.TipoApp || '').toLowerCase() === 'app interno') {
      const msg = "Para restaurar este aplicativo, altere o Tipo para 'app comercial' antes.";

      // Inline visual alert
      let alertDiv = document.getElementById('modal-inline-alert');
      if (!alertDiv) {
        alertDiv = document.createElement('div');
        alertDiv.id = 'modal-inline-alert';
        alertDiv.style.color = '#ff5555';
        alertDiv.style.backgroundColor = 'rgba(255, 85, 85, 0.1)';
        alertDiv.style.border = '1px solid #ff5555';
      ...
      setTimeout(() => { if (alertDiv) alertDiv.style.display = 'none'; }, 6000);
...
window.loadData = () => {
  console.log('=== INÍCIO CARREGAMENTO DE DADOS ===');
  ...
  fetch('data/apps_output.csv?t=' + Date.now()) // timestamp para evitar cache
    ...
    .then(text => {
      ...
      if (state.lastCsvText !== text) {
        ...
        console.log('CSV mudou, chamando initFromText...');
        initFromText(text);
      } else {
        console.log('CSV não mudou, usando dados existentes');
      }
    })
    .catch(() => {
      console.log('ERRO ao carregar CSV, usando fallback...');
      // Se falhar e ainda não tiver dados carregados, usa o fallback
      if (!state.data || state.data.length === 0) {
        console.log('Carregando defaultCsv...');
        initFromText(defaultCsv);
      }
    });
};
...
(function init() {
  ...
  // Polling desabilitado para não interferir nas observações
